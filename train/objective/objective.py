...
#     # === 把 worker_tag / batch_size 寫進 trial 屬性，便於事後追蹤 ===
#     trial.set_user_attr("worker_tag", worker_tag or "single")
#     trial.set_user_attr("batch_size", int(cfg["train"]["batch_size"]))

#     # === 將不同 worker 的輸出分到不同子資料夾 ===
#     if worker_tag:
#         trial_dir = run_dir / worker_tag / f"trial_{trial.number:03d}"
#     else:
#         trial_dir = run_dir / f"trial_{trial.number:03d}"
...
#     # ==== 分數差分（FFD）超參 ====
#     if str(cfg["label"].get("ret_type", "")).lower() == "fractionally":
#         cfg["label"]["fracdiff"]["d"] = suggest_float(
#             trial,
#             "fracdiff.d",
#             cfg["label"]["fracdiff"]["d"]
...
#     # ------------------------------------------------------------------
#     # 在所有 suggest_* 都完成後，先把「本次取樣參數」覆寫到一份 YAML 存起來
#     # 只覆寫 trial.params 內出現的鍵；其餘沿用 base_cfg
#     # 非 dot-path 名稱用 keymap 映射回 YAML 路徑
#     # ------------------------------------------------------------------
#     keymap = {
#         # 1. train:
#         "lr": "train.lr",
...
#         "threshold": "train.threshold",  # 僅 threshold_mode=fixed 時會存在於 trial.params
#         # 2. sequence
...
#         "minute_hidden": "model.minute_hidden",
#         "minute_layers": "model.minute_layers",
#         "minute_dropout": "model.minute_dropout"
#     }
#     used_cfg_path = save_trial_config(
#         base_cfg=base_cfg,                # 原始 YAML（未修改）
#         trial_params=trial.params,        # 本次 trial 取樣到的所有鍵（含 dot-path）
#         trial_dir=str(trial_dir),         # 該 trial 的輸出資料夾
#         keymap=keymap,                    # 名稱 ↔ dot-path 對應表
#         extra_meta={
#             "trial_number": trial.number,
#             "study_name": trial.study.study_name,
#             "worker_tag": worker_tag or "single",
#             "task_type": get_task_type(cfg),
#         },
#         filename="config.used.yaml",
#     )
#     print(f"[objective] wrote used config -> {used_cfg_path}")
...
#     mean_score = float(np.mean(fold_scores))
#     trial.set_user_attr("selected_features", feature_columns if 'feature_columns' in locals() else None)
#     trial.set_user_attr("n_features", int(len(feature_columns)) if feature_columns is not None else None)
#     trial.set_user_attr("fold_scores", fold_scores)
#     trial.set_user_attr("task_type", task_type)
#     trial.set_user_attr("primary_metric", str(cfg["objective"]["primary_metric"]))
#     trial.set_user_attr("direction", str(cfg["objective"]["direction"]))
