...
#     decision_mode: str = "mean_prob",   # "mean_prob" | "fold_vote"
# ) -> str:
#     """
#     以某個 trial 的各 fold 最佳模型，對 [date_start,date_end] 區間的事件做推論並回寫 CSV。
#     - 僅對 (keep_sides, 日期遮罩) 的子集做推論，但輸出 CSV 保留全部 rows。
#     - 預設「各 fold 機率平均 → 以平均驗證門檻判斷」；也可設定 decision_mode='fold_vote'：
#       「各 fold 用各自 best_val_thresh 產生 0/1 → 多數決」。
#     - 產出欄位：{pred}（0/1）、{pred}_p0、{pred}_p1、{pred}_thr（fold_vote 模式下本欄為 NaN）。
#     """
...
#     # ---- dataloader / 推論通用設定 ----
#     # 允許 post_infer 覆寫較小的 batch_size，避免 OOM
#     bs = int(
#         ((cfg.get("post_infer", {}) or {}).get("tbm_concat", {}) or {}).get(
...
#         rid_counter = defaultdict(int)  # key: t0_utc -> 已經配過幾次（用作 rid_lists_by_t0 的索引）
...
#         # 2) scaler 擬合 + 全表 transform（timesafe）/子集 transform
...
#                 #     # 事件鍵：原始 t0_utc；用 rid_queue 逐一彈出對應 rid
...
#                 #     rlist = rid_queue.get(key_t0, [])
#                 #     if not rlist:
#                 #         # 找不到對應 rid，跳過（可能是資料前處理導致某些事件無法建序列）
#                 #         continue
#                 #     rid = rlist.pop(0)  # FIFO
...
#     col_thr = f"{output_column}_thr"

#     # 找到所有有任一 fold 機率的 rid（有效推論事件）
#     rid_union = sorted(set().union(*[set(d.keys()) for d in per_fold_prob.values()])) if per_fold_prob else []

#     # 1) 預設：平均機率 + 單一門檻
#     pred_records: List[Tuple[int, float, float, float, int]] = []  # (rid, p0, p1, thr_used, pred)
#     vote_records: List[Tuple[int, int, int]] = []  # (rid, votes_0, votes_1) for fold_vote summary
...
#     #                 continue
#     #             p1 = d[rid]
#     #             f_thr = thr_list[f_idx] if (f_idx < len(thr_list)) else thr_used_global
#     #             yhat = int(p1 >= float(f_thr))
#     #             if yhat == 1:
#     #                 votes_1 += 1
#     #             else:
#     #                 votes_0 += 1
...
#     #         pred = 1 if (votes_1 > votes_0) else 0
#     #         pred_records.append((rid, p0_mean, p1_mean, np.nan, pred))  # thr 用不到 → NaN
#     #         vote_records.append((rid, votes_0, votes_1))
#     #     else:
#     #         # mean_prob：平均機率 → 與單一 thr 判斷
#     #         p1_mean = float(np.mean(probs))
#     #         p0_mean = float(1.0 - p1_mean)
#     #         pred = int(p1_mean >= thr_used_global)
#     #         pred_records.append((rid, p0_mean, p1_mean, thr_used_global, pred))

#     # # 組成可 merge 的 DataFrame
#     # pred_df = pd.DataFrame(pred_records, columns=["__rid", col_p0, col_p1, col_thr, col_pred])
#     # if dm == "fold_vote":
#     #     vote_df = pd.DataFrame(vote_records, columns=["__rid", f"{output_column}_votes_0", f"{output_column}_votes_1"])
#     #     pred_df = pred_df.merge(vote_df, on="__rid", how="left")

#     # # ---- 合併回「完整」tbm_all（保留所有 side）----
#     # tbm_out = tbm_all.merge(pred_df, on="__rid", how="left", validate="one_to_one")

#     # # 想讓 pred_p0 在 pred_p1 左側，嘗試調整欄位順序（若已存在就尊重原順序）
#     # def _reorder_cols(df: pd.DataFrame, names_in_order: List[str]) -> pd.DataFrame:
#     #     exist = [c for c in names_in_order if c in df.columns]
#     #     others = [c for c in df.columns if c not in exist]
#     #     return df[others[:]] if not exist else df[[*(c for c in df.columns if c not in exist), *exist]]

#     # # 這裡不強行整表重排，只確保 pred 欄位群彼此順序（p0→p1→thr→pred）
#     # pred_group = [col_p0, col_p1, col_thr, col_pred]
#     # exist_pred_group = [c for c in pred_group if c in tbm_out.columns]
#     # if exist_pred_group:
#     #     # 把這群欄位先拔掉再接到最後（維持相對順序）
#     #     base_cols = [c for c in tbm_out.columns if c not in exist_pred_group]
#     #     tbm_out = tbm_out[base_cols + exist_pred_group]
...
#     # tbm_out.to_csv(save_to_path, index=False)

#     # # ---- summary ----
#     # try:
#     #     import json
#     #     mask_out = tbm_out["__rid"].isin(rid_union)
#     #     n_total_rows = int(len(tbm_out))
#     #     n_predicted = int(mask_out.sum())
#     #     ps = tbm_out.loc[mask_out, col_p1].astype("float64")
#     #     summary = {
...
#     #         "decision_mode": dm,
#     #         "thr_used_global": (None if dm == "fold_vote" else float(thr_used_global)),
#     #         "thr_mean": float(thr_mean),
#     #         "thr_list_per_fold": [float(x) for x in thr_list],
#     #         "rows_in_output": n_total_rows,
#     #         "events_predicted": n_predicted,
#     #         "pred_ones": int((tbm_out.loc[mask_out, col_pred] == 1).sum()) if col_pred in tbm_out.columns else None,
#     #         "pred_zeros": int((tbm_out.loc[mask_out, col_pred] == 0).sum()) if col_pred in tbm_out.columns else None,
#     #         "prob_mean": float(ps.mean(skipna=True)) if n_predicted else None,
#     #         "prob_min": float(ps.min(skipna=True)) if n_predicted else None,
#     #         "prob_max": float(ps.max(skipna=True)) if n_predicted else None,
#     #     }
...
#     thr_mean = float(np.mean(thr_list)) if len(thr_list) else 0.5
#     thr_used_global = float(threshold_override) if (threshold_override is not None) else thr_mean

#     # 逐 fold 門檻（hard 用），若該 fold 無門檻資訊則退回全域門檻
#     # thr_map 的索引與 enumerate(fold_models) 的 fold_idx 對齊
#     thr_map: Dict[int, float] = {i: (thr_list[i] if i < len(thr_list) else thr_used_global)
#                                  for i, _ in enumerate(fold_models)}

#     # 蒐集兩組結果
#     prob_records: List[Tuple[int, float, float, float, int]] = []  # rid, p0, p1, thr, yhat_prob
#     vote_records: List[Tuple[int, float, float, int, int, int]] = []  # rid, p0, p1, votes_0, votes_1, yhat_vote
...
#             vote_records.append((rid, p0_mean, p1_mean, v0, v1, yhat_vote))
...
#         vote_df = pd.DataFrame(vote_records, columns=["__rid", col_vote_p0, col_vote_p1, col_vote_v0, col_vote_v1, col_vote_pred])
#         out_df = out_df.merge(vote_df, on="__rid", how="left", validate="one_to_one")

#     # 欄位相對順序調整：各群內維持 p0→p1→thr/votes→pred
...
#     if dm in {"fold_vote", "both"}:
#         out_df = _move_group_to_tail(out_df, [col_vote_p0, col_vote_p1, col_vote_v0, col_vote_v1, col_vote_pred])

#     # ---- 輸出 ----（沿用原本命名規則；both 模式檔名不變）
#     if save_to_path is None:
