...
#             out["DOW_COS"] = np.cos(2*np.pi+dow/7.0).astype(np.float32) if False else np.cos(2*np.pi*dow/7.0).astype(np.float32)
#         return pd.DataFrame(out, index=self.df.index)
...
#         2. inputs:
#            - lib(IndicatorLibrary): 指標庫實例（提供 builders）。
...
#         """
#         1. 說明:
#            從 plan 中篩出 enabled=True 的特徵規格。

#         2. inputs:
#            - plan(dict): 需包含 "features": [{"name", "kwargs", "enabled"}, ...] 的結構。

#         3. return:
#            - List[dict]: 僅包含啟用的特徵規格清單。
#         """
...
#         """
#         1. 說明:
#            呼叫對應 builder 產生單一特徵 DataFrame，並做 shift(1) 以避免未來資訊洩漏。

#         2. inputs:
#            - name(str): 指標名稱（對應 self.lib.builders 的 key）。
#            - kwargs(dict): 指標建構參數（可為空）。

#         3. return:
#            - pd.DataFrame: 單一特徵（或一組同類特徵）之欄位，索引為時間，已 shift(1)。
#         """
#         key = str(name).upper()
...
#         """
#         1. 說明:
#            依設定挑選 minute 級別（如 m_*）的直通欄位，作為額外保留的特徵。

#         2. inputs:
#            - cfg(dict|None): 全域設定，需於 cfg["features"]["min_trade_feat"] 指定白名單；
#                              若為 None 則不保留任何直通欄。

#         3. return:
#            - List[str]: 應該被直通保留的欄位名稱列表（依 self.lib.df 中存在者）。
#         """
#         if cfg is None:
#             return []
...
#         """
#         1. 說明:
#            將分段特徵拼接、清理（處理 inf→NaN、astype float32、依設定 dropna）、檢查欄位重複。

#         2. inputs:
#            - parts(List[pd.DataFrame]): 多個已對齊索引的特徵分段表。
#            - cfg(dict|None): 全域設定；若 cfg["features"]["dropna"]=True 則會刪除含 NaN 的列。

#         3. return:
#            - pd.DataFrame: 最終的特徵表（float32），若無 parts 則回空 DataFrame。
#         """
#         if not parts:
#             return pd.DataFrame()
...
#     # --- compute（僅計算；不使用快取與 manifest）---
#     def compute(self, plan: Dict[str, Any], cfg, *, load_if_exists: bool = True) -> pd.DataFrame:
#         """
#         1. 說明:
#            依 plan 計算全部啟用的特徵，統一對各欄位 shift(1) 防洩漏，並（可選）附加 minute 直通欄位。
#            本方法不讀寫任何快取檔，參數 load_if_exists 僅為介面相容。

#         2. inputs:
#            - plan(dict): 特徵計畫，內含 "features"：[{name, kwargs, enabled}, ...]。
#            - cfg(dict): 全域設定（讀取 "features.dropna"、"features.min_trade_feat" 等）。
#            - load_if_exists(bool): 相容參數，無實際作用。

#         3. return:
#            - pd.DataFrame: 完整特徵表（float32），索引為時間，列視 dropna 設定可能被裁剪。
#         """
#         feat_list = self._enabled_features(plan)
#         if not feat_list:
#             raise ValueError("計畫沒有任何 enabled=True 的 features。")

#         # 1) 計算所有 feature => parts
...
#     # --- 欄位查詢（依當前 features 規格；不使用磁碟 manifest）---
#     def _infer_columns_for_plan(self, plan: Dict[str, Any], cfg) -> List[str]:
#         """
#         1. 說明:
#            以實際呼叫 builders（但只取欄名、不做最終整併）推導此 plan 會產生的欄位集合，
#            並依 cfg 決定是否附加 minute 直通欄位；保持輸出順序且去重。

#         2. inputs:
#            - plan(dict): 特徵計畫（同 compute）。
#            - cfg(dict|None): 全域設定（讀取 "features.include_passthrough"、"features.min_trade_feat"）。

#         3. return:
#            - List[str]: 欄位名稱列表（按出現順序且無重複）。
#         """
#         feat_list = self._enabled_features(plan)
#         if not feat_list:
#             return []
...
#         """
#         1. 說明:
#            取得此 plan 會產生的全部特徵欄位名稱（可含 minute 直通欄），
#            不依賴任何磁碟 manifest，完全以 in-memory 推導。

#         2. inputs:
#            - plan(dict): 特徵計畫。
#            - cfg(dict|None): 全域設定（控制 include_passthrough 等）。
...
#         """
#         1. 說明:
#            回傳單一特徵規格（或在給定 plan 的上下文中）所產生的欄位名稱集合。

#         2. inputs:
#            - name(str): 指標名稱（對應 builders 的 key）。
#            - kwargs(dict|None): 指標參數（預設 {}）。
#            - plan(dict|None): 若提供，則以此 plan 的規則推導；否則使用僅包含該特徵的臨時 plan。
#            - cfg(dict|None): 全域設定（控制 include_passthrough 等）。

#         3. return:
#            - List[str]: 欄位名稱列表（依實際 builder 回傳的欄位）。
#         """
...
#         if missing_cols:
#             print(f"[WARN] {len(missing_cols)} feature columns missing in X (likely all-NaN before dropna or builder returned none):\n  {missing_cols[:20]}{' ...' if len(missing_cols)>20 else ''}")
...
#     else:
#         # default to parquet if no recognized suffix
...
#     main()
# train/build_feature_loader/indicators.py
from __future__ import annotations
from pathlib import Path
from typing import Dict, Any, Union, Iterable, List
import yaml
