...
class StrategyAnalyzer:
    ...
    def _simulate_market_evolution(self):
        ...
        這個過程只更新狀態，不產生訊號。
        """
        log.info(f"[{self.symbol}/{self.timeframe}] Starting market evolution simulation...")
        ...
        log.info(f"[{self.symbol}/{self.timeframe}] Simulation complete. Total levels managed: {len(self.levels)}.")
    ...
    def _update_existing_levels(self, candle: pd.Series):
        ...
        for level in self.levels:
            ...
            if is_broken_up or is_broken_down:
                ...
                level.is_valid = True
                log.debug(f"Level at {level.price} flipped to {level.type} at {candle.name}")
            ...
            if is_tested_by_wick:
                ...
                log.debug(f"Level at {level.price} was wick‑tested and invalidated at {candle.name}")
    ...
    def _check_last_candle_for_signal(self):
        ...
        """
        log.info(f"[{self.symbol}/{self.timeframe}] Checking last candle ({self.last_candle.name}) for signals...")
        ...
        for level in self.levels:
            ...
            log.warning(f"!!! [{self.symbol}/{self.timeframe}] Fresh {signal_type} Signal Found !!!")
    ...
    def analyze(self) -> List[Dict[str, Any]]:
        ...
        log.info(f"Analysis complete for {self.symbol}/{self.timeframe}. Found {len(self.signals)} new signal(s).")
        return self.signals