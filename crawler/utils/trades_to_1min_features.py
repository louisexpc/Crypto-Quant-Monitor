...
def main(args: argparse.Namespace | None = None):
    ...
    file_pattern = os.path.join(args.input_dir, f"{args.symbol}-trades-*.zip")
    ...
    for p in files:
        ...
        if not d:
            continue
        if d >= args.start and d <= args.end:
            chosen.append(p)
    ...
    out = full[cols_order].copy()

    # 新增對齊欄位
    out.insert(0, "datetime", out.index.map(lambda ts: ts.isoformat()))
    out.insert(
        1,
        "timestamp",
        (out.index.astype("int64") // 10**9).astype("int64"),
    )
    out = out.reset_index(drop=True)
    out.to_csv(args.output_csv, index=False)
    print(f"完成！輸出：{args.output_csv}  共 {len(out):,} 列")


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Convert Binance trades zips into 1min features")
    ap.add_argument("--input_dir", default=DEFAULT_INPUT_DIR)
    ap.add_argument("--symbol", default=DEFAULT_SYMBOL)
    ap.add_argument("--start", default=DEFAULT_START)
    ap.add_argument("--end", default=DEFAULT_END)
    ap.add_argument("--output_csv", default=DEFAULT_OUTPUT)
