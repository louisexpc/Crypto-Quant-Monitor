...
def main():
    ...
    ap.add_argument('--timeframes', nargs='*', default=None,
                    help='Override timeframes (space separated). Default follows YAML monitoring entries.')
    ...
    cli_timeframes = args.timeframes

    if mon:  # YAML 驅動，可由 CLI timeframes 覆蓋
        pairs = []
        for item in mon:
            tfs = cli_timeframes if cli_timeframes else item.get('timeframes')
            if not tfs:
                raise SystemExit(f"monitoring entry {item} missing timeframes and no --timeframes provided")
            pairs.append((item['symbol'], tfs))
    else:
        if not args.symbols or not cli_timeframes:
            raise SystemExit("Provide --symbols and --timeframes when no --config is given.")
        pairs = [(s, cli_timeframes) for s in args.symbols]
